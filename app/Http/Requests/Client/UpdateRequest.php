...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'dni' => 'required|email|string|min:8|max:8|unique:clients,dni,'.$this->route('client'),
            'nit' => 'required|string|min:11|max:11|unique:clients,nit,'.$this->route('client'),
            'address' => 'nullable|string|max:255',
            'phone' => 'string|min:9|max:9|unique:clients,phone,'.$this->route('client'),
            'email' => 'string|max:255|unique:clients|email,'.$this->route('client').'|email:rfc,dns',
        ];
    }
